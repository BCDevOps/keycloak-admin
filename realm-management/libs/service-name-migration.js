...
} = require('./service-name-migration-utils');

const { IDP_TERMS, KC_MIGRATION_ROUTES, MODE } = require('../constants');

const ssoServiceNameMigration = async (kcAdminClient) => {
  try {
    ...
    // -------------------------------------------------------------------------------------------
    // 7. Rollback: switch new and old URL and re-run script
  } catch (err) {
    throw Error(err);
  }
};

module.exports = { ssoServiceNameMigration };
