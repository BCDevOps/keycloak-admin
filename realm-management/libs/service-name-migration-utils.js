...
const urlReplacer = (jsonContent, targetString, replacement) => {
  ...
  if (!stringifiedContent.includes(targetString)) {
    console.warn('----No target string found from:');
...
const updateIdpAppClient = async (clientId, idp, kcAdminClient, newRoute, oldRoute) => {
  ...
  if (idpClients.length !== 1) {
    throw Error(`----Not expected clients found #${idpClients.length}!`);
  }
  const idpClient = idpClients[0];
  await fs.outputJson(
    `${OUTPUT_PATH}/clients/${idpClientRef.realm}/${idpClientRef.clientId}.json`,
    idpClient,
  );
...
const updateAgentIdpMappers = async (kcAdminClient, mapperContent, mapperRef) => {
  const newMapperContent = mapperContent;
